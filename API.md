...
## 17. WebRTC Egress (Zero-Latency Live Mode listening)

Negotiates a zero-latency egress (outward) WebRTC connection to receive the station's real-time audio output using high-fidelity Opus over UDP.
...
## 18. WebRTC Ingress (Browser Microphone Live Publishing)

Allows authorized DJs to broadcast their **live microphone audio directly from any web browser** to the station's Priority Channel (Channel 0) with automatic ducking of other active channels.

```
POST /webrtc/publish?id={station_id}&token={webrtc_ingress_token}
Content-Type: application/json
```
...
*Note: Ingress requires a valid security token configured via `webrtc_ingress_token` in `station.cfg`.*

---

## 19. Manual Mixing Deck Mode

Toggles the station between **Auto-DJ Mode** (default) and **Manual Mixing Deck Mode**. In manual mode, the automatic track schedule is paused, giving full granular control of channels 1 & 2 to the DJ.
...
## 20. Manual Deck Playback Controls

These endpoints are only available when **Manual Mixing Deck Mode** is active (`"manual": true`).

### 20.1 Load Standby Track
Loads an audio file into a specific channel buffer on standby, preparing it for playback.
```
POST /mixer/standby
Content-Type: application/json

{
    "station_id": "radio1",
    "channel": 1,
    "file": "/path/to/music/next_track.mp3"
}
```

### 20.2 Play Standby Track
Instantly starts playback of the preloaded standby track on a channel.
```
POST /mixer/play
Content-Type: application/json
...
### 20.3 Trigger Manual Crossfade (Mixing)
Fades out one playlist channel (e.g. Channel 1) while fading in the other playlist channel (e.g. Channel 2) smoothly.
```
POST /mixer/mixing
Content-Type: application/json
...
### 20.4 Pause / Resume Track
Pauses or resumes playback on a specific mixer channel.
```
POST /mixer/pause
Content-Type: application/json
...
### 20.5 Seek / Rewind Track
Seeks to a specific position (in seconds) or offsets from the current playback position on a channel.
```
POST /mixer/rewind
Content-Type: application/json
...
## 21. Advanced Operations & Diagnostics

### 21.1 Live Configuration Reload
Reads `station.cfg` from disk and applies changes (e.g. metadata text, RTMP, WebRTC, crossfades) instantly without stopping the station or disconnecting listeners.
```
POST /station/reload
Content-Type: application/json
...
### 21.2 Skip Channel Track
Instantly terminates the track currently playing on a specific channel, clearing its buffers.
```
POST /mixer/skip
Content-Type: application/json
...
### 21.3 Restart Mixing Engine
Clears the entire audio mixer buffers and re-initializes the live mixing stream. Used for low-level diagnostic recovery.
```
POST /mixer/restart
Content-Type: application/json
