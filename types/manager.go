...
func (m *Manager) createStationWithDir(id string, outputDir string, playlistDir string) (*StationRunner, error) {
	...
	if playlistDir != "" {
		files, err := filepath.Glob(filepath.Join(playlistDir, "*.mp3"))
		if err == nil && len(files) > 0 {
			runner.QueueMgr.ReplacePlaylist(files)
			log.Printf("[Manager] Station %s auto-injected %d files from %s", id, len(files), playlistDir)
		} else {
			log.Printf("[Manager] Station %s: no mp3 files found in %s", id, playlistDir)
		}
	}
